...
```bash
# 1) 进入项目目录
cd /www/wwwroot/Server_Probe/server-probe

# 2) 查看当前分支和本地改动
git branch --show-current
git status

# 3) 拉取远程最新代码（以 main 分支为例）
git fetch origin
git pull --rebase origin main
```

如果第 3 步提示有本地修改冲突，可先暂存本地改动再更新：

```bash
git stash push -m "temp local changes before deploy"
git pull --rebase origin main
git stash pop
```
...
```bash
python3 -m pip install -r requirements.txt
# 宝塔中点击“重启”项目，或在终端重启你的进程管理器（systemd/supervisor）
```
